...
export function createServerWs() {
    return async (dispatch, getState) => {
        ...
        console.log(require('socket.io-client')(serverWsUrl, { transports: ['websocket'], query: `name=123`, secure: false, rejectUnauthorized: false }));
        const socket = !serverWs
            ? require('socket.io-client')(serverWsUrl, { transports: ['websocket'], query: `name=123`, secure: false, rejectUnauthorized: false })//"http://botmarket24.online:2222"//("ws://23.105.248.184:55441/")("ws://localhost:55441/")//http://localhost:5000//https://botmarket24.online
            : serverWs;
...
async function historyData2(binanceApiKey, binanceSecret, symbol, from) {
    ...
    const [res] = await Promise.all([axios.get(`https://fapi.binance.com/fapi/v1/userTrades?symbol=${symbol}&startTime=${from}&timestamp=${unixTimestamp.toString()}&signature=${signature}`,
        {
            headers: headers
        }).catch((error) => {
        if (error.response) {
